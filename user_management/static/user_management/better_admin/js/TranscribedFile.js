...
var MediaSrc ="";
var FileID ="";
GetTranscribedMediaFile();

function GetTranscribedMediaFile() {
    ...
    MediaSrc = ("../../itrans/get_upload_file/?file_id=" + FileID);
...
if( MediaType.indexOf('audio') >= 0){
    ...
    console.log(MediaSrc + "  Audiosrcccc audio" );



    $("#MediaPlayer").append('<audio controls id="MediaSource"><source src="" type='+MediaType+' preload="auto" data-setup="{}">Your browser does not support the audio element.</audio>');
    $("#MediaPlayer source").attr("src", MediaSrc);
}
  else{
    console.log(MediaSrc + "  MediaSrcMediaSrcMediaSrc video" );

    $("#MediaPlayer").append('<video controls id="MediaSource"><source src="" type='+MediaType+' preload="auto" data-setup="{}">Your browser does not support the video element.</video>');
    $("#MediaPlayer source").attr("src", MediaSrc);
  }


// $("#MediaPlayer")
var player = document.getElementById ("MediaSource");
var onClickST = "";
var onClickET = "";
var TimeCheck = "";
...
player.addEventListener('canplaythrough', function () {
    $("#TranscibedFileTextPreviewNew").css("visibility","visible");
    // Add code to fade out the loading screen.
});
player.ontimeupdate = function() {TimeUpdateFunction()};

function TimeUpdateFunction() {
  // Display the current position of the audio in a p element with id="demo"
  MediaCurrentTime = player.currentTime;
  console.log("The data-id of clicked item is: " + onClickET + " MEdia Time " + MediaCurrentTime);
  ...
  $.each(TimeCheck, function (index, value) {

      EndTimeValue = ((value).attr("endTime"));
      StartTimeValue = ((value).attr("starttime"));
      if (EndTimeValue != undefined) {

        

          if ((StartTimeValue <= MediaCurrentTime) && (EndTimeValue <= MediaCurrentTime)) {


              $(value).parent().addClass('active');
              $(value).parent().next().addClass('active');

              ExStartTime = $(value).prev().attr("starttime");
              ExEndTime = $(value).prev().attr("endtime");
              NextStartTime = $(value).next().attr("starttime");
              NextEndTime = $(value).next().attr("endtime");
              //   $(value).prev().prev().prev().prevAll().css('background-color', 'white');

              //   $(value).parent().prev().prevAll().removeClass('active');
              $(value).parent().next().nextAll().removeClass('active');

              if(EndTimeValue == NextStartTime){
   ...
   .catch(function (error) {
    ...
    $(document).on("click", "#UpdateTranscribedText", function () {
